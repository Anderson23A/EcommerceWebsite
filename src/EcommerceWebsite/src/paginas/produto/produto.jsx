...
export default function ProdutoPagina() {
    ...
    const carregar_dados = async () => {
        const res = (await servidor.get('/produtos/pegar_produto/', { params: { id: params.produtoId, headers: {'Content-Type': 'application/json'} },  }))
        const data = res['data']
    ...
    const comprar_produto = async () => {
        const res = (await servidor.post('/produtos/criar_checkout/', {
            quantidade: quantidade,
            produtoId: params.produtoId,
        }, {headers: {'Content-Type': 'application/json'}}))['data']
        const checkoutId_data = res['checkoutId']
        const stripePromise = await loadStripe('pk_test_51Ouudh00R55810eE4GI2TM9GAba1xLOBdIISXlIViD1cvmnTQ0Ujehgw9BSKkqUzm5LIwPGUtKIhrcHg78gb0z2Q00cj0WKX2G')
                        ...
                        <Col lg>
                            <div className='produto-detalhes'>
                                <div className='detalhes'>
                                    ...
                                    <p className='produto-descricao'>{produtoDetalhes.descricao}</p>
                                </div>
